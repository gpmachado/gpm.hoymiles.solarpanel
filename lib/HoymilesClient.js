...
class HoymilesClient {
  ...
  // Version string format from hoymiles.py: generate_dtu_version_string()
  _dtuVersionString(n, prefix) {
    const v1 = Math.floor(n / 4096);
    const v3 = Math.floor(n / 256) % 16;
    const v2 = n % 256;
    return `${prefix}${String(v1).padStart(2,'0')}.${String(v3).padStart(2,'0')}.${String(v2).padStart(2,'0')}`;
  }

  // cmd 0x01 — APPInfoDataReqDTO
  // Returns: { signalStrength, swVersion, hwVersion, wifiVersion }
  async getAppInfo() {
    ...
    const payload = Buffer.from(pb.APPInfoDataResDTO.encode(req).finish());
    const frames  = await tcpRequest(this._host, 0x01, payload, this._seq++, this._log);

    if (!frames || frames.length === 0 || !frames[0].payload.length) return null;

    const msg     = pb.APPInfoDataReqDTO.decode(frames[0].payload);
    const dtuInfo = pick(msg, 'dtu_info', 'dtuInfo') || {};
    const signal  = toNumber(pick(dtuInfo, 'signal_strength', 'signalStrength'));
    const sw      = toNumber(pick(dtuInfo, 'dtu_sw_version',  'dtuSwVersion'));
    const hw      = toNumber(pick(dtuInfo, 'dtu_hw_version',  'dtuHwVersion'));
    const wifiVer = pick(dtuInfo, 'wifi_version', 'wifiVersion') || '';

    return {
      signalStrength: signal,                           // % (0-100)
      swVersion:      sw ? this._dtuVersionString(sw, 'V') : wifiVer,
      hwVersion:      hw ? this._dtuVersionString(hw, 'H') : '',
    };
  }

  // cmd 0x09 — GetConfigReqDTO
  // Returns: { wifiSsid, dtuApSsid, serverHost }
  async getConfig() {
    const req = pb.GetConfigResDTO.create({
      offset: 28800,
      time: Math.floor(Date.now() / 1000) - 60,
    });
    const payload = Buffer.from(pb.GetConfigResDTO.encode(req).finish());
    const frames  = await tcpRequest(this._host, 0x09, payload, this._seq++, this._log);

    if (!frames || frames.length === 0 || !frames[0].payload.length) return null;

    const msg = pb.GetConfigReqDTO.decode(frames[0].payload);
    return {
      wifiSsid:   pick(msg, 'wifi_ssid',          'wifiSsid')         || '',
      dtuApSsid:  pick(msg, 'dtu_ap_ssid',        'dtuApSsid')        || '',
      serverHost: pick(msg, 'server_domain_name',  'serverDomainName') || '',
    };
  }
  ...
  async getData() {
    ...
    // Only log when value changes to avoid spam every 60s
    if (warnRaw !== this._lastWarnRaw) {
      this._lastWarnRaw = warnRaw;
      if (warnRaw > 0) {
        this._log(`warning_number changed: ${warnRaw} — ${isRealAlarm ? 'REAL FAULT' : 'informational (ignored)'}`);
      }
    }
